...
static void bgr565_to_i420(const uint16_t *src,
...
void encoder_task_start_after_vglite(void)
{
    ...
    cy_rslt_t result = cy_rtos_thread_create(&encoder_thread, &cm55_encoder_task,
                                             ENCODER_TASK_NAME, NULL,
...
void encoder_on_display_frame_done(prediction_od_t *pred)
{
    ...
    vg_lite_error_t vs = vg_lite_blit(&encoder_frame,
                                      &dvp_bgr565_frames[active_frame],
                                      NULL,
                                      VG_LITE_BLEND_NONE,
                                      0,
...
void cm55_encoder_task(void *arg)
{
    ...
    for (;;) {
        ...
        int status = H264E_encode((H264E_persist_t *)encoder_persist,
                                  (H264E_scratch_t *)encoder_scratch,
                                  &encoder_run_param,
                                  &encoder_io,
                                  &coded_data,
                                  &coded_size);
        ...
        /* 3. Publish into the cross-core ring.  CM33 consumer is not
         * built yet; for now the ring is a producer-only audit buffer.
         * Keyframes land at GOP boundaries when frame_type=DEFAULT. */
        bool is_idr = ((encoder_frame_seq % ENCODER_GOP) == 0);
        video_ring_slot_t *slot = video_ring_acquire_write_slot();
        uint32_t copy_len = (uint32_t)coded_size;
        if (copy_len > VIDEO_RING_SLOT_BYTES) {
            copy_len = VIDEO_RING_SLOT_BYTES;
        }
        memcpy(slot->payload, coded_data, copy_len);
        video_ring_publish(slot, copy_len, (uint32_t)t_start, is_idr);
        ...
        if (frames >= 30) {
            ...
            printf("[enc] inf_fps=%.2f dsp_fps=%.2f in_fps=%.2f enc_fps=%.2f "
                   ...
                   (unsigned)(total_bytes / frames),
                   (int)is_idr,
                   (unsigned)h->write_index,
                   (unsigned)h->read_index,
                   (unsigned)h->dropped,
