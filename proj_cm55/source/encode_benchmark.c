...
*   None
*******************************************************************************/
static void encode_benchmark_fill_synthetic(uint8_t *yuv, int width, int height, int frame_idx ) {
    uint8_t *y_plane = yuv;
...
static void encode_benchmark_run_case(const encode_benchmark_case_t *test_case) {
    ...
    if ((sizeof_persist > ENCODE_BENCHMARK_PERSIST_BYTES) || (sizeof_scratch > ENCODE_BENCHMARK_SCRATCH_BYTES)) {
    ...
    run_param.qp_min = 20;
    run_param.qp_max  = 45;
    run_param.desired_frame_bytes = (test_case->bitrate_bps / 8) / ENCODE_BENCHMARK_TARGET_FPS;
    run_param.desired_nalu_bytes = 0;

    t_start = xTaskGetTickCount();

    for (frame_idx = 0; frame_idx < ENCODE_BENCHMARK_FRAMES; frame_idx++) {
        encode_benchmark_fill_synthetic(
            encode_benchmark_yuv,
            test_case->width,
            test_case->height,
            frame_idx
        );
        ...
        status = H264E_encode(
            (H264E_persist_t *)encode_benchmark_persist,
    ...
    measured_fps = (avg_ms_per_frame > 0.0f)? (1000.0f / avg_ms_per_frame) : 0.0f;
...
void cm55_encode_benchmark_task(void *arg) {
    ...
    size_t idx;
    for (idx = 0; idx < (sizeof(encode_benchmark_cases) / sizeof(encode_benchmark_cases[0])); idx++ ) {
