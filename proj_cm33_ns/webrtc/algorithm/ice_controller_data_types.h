...
#else
#define ICE_CONTROLLER_CONNECTIVITY_TIMER_INTERVAL_MS ( 2000 )
...
#define ICE_CONTROLLER_CONNECTIVITY_CHECK_TIMEOUT_MS ( 120000 )
...
/* TLS stream reassembly buffer size.  Allocated on-demand for TLS sockets
 * only (typically 1 TURN relay socket).  4096 bytes matches the per-recv
 * read size and comfortably holds multiple coalesced STUN responses. */
#define ICE_CONTROLLER_TLS_RX_BUF_SIZE  ( 4096 )
